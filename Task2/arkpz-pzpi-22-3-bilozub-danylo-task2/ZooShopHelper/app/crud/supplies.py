...
def get_supplies(db: Session, supplies_id: int):
    return db.query(Supplies).filter(Supplies.supplies_id == supplies_id).first()

def get_all_supplies(db: Session, skip: int = 0, limit: int = 100):
    return db.query(Supplies).offset(skip).limit(limit).all()

def update_supplies(db: Session, supplies_id: int, supplies: SuppliesUpdate):
    db_supplies = db.query(Supplies).filter(Supplies.supplies_id == supplies_id).first()
    if db_supplies:
...
def delete_supplies(db: Session, supplies_id: int):
    db_supplies = db.query(Supplies).filter(Supplies.supplies_id == supplies_id).first()
    if db_supplies:
